...
def get_sector_spin_html(df_m):
    ...
    if df_m is None or df_m.empty or 'Name' not in df_m.columns:
        return ''

    # ── [실시간성 강화] 네이버 금융 실시간 체결가 우선 사용 ──────────────────
    # FDR StockListing은 장 시작 초반(09:00~09:30)에 전날 종가를 반환하는 문제가 있어
    # 순환매 지도의 등락률/거래대금이 실시간을 반영하지 못하는 현상 발생.
    # 네이버 금융은 장중 60초 주기로 실시간 체결가를 제공하므로 이를 우선 사용.
    all_stocks_in_map = tuple(set(s for stocks in SECTOR_MAP.values() for s in stocks))
    _now_kst_spin = datetime.now(timezone(timedelta(hours=9)))
    _hm_spin = _now_kst_spin.hour * 100 + _now_kst_spin.minute
    _is_market_spin = _now_kst_spin.weekday() < 5 and 900 <= _hm_spin <= 1540

    # 장중에만 네이버 실시간 조회 시도 (장외에는 df_m 기반으로 폴백)
    naver_prices = {}
    if _is_market_spin:
        try:
            naver_prices = fetch_naver_realtime_sector_prices(all_stocks_in_map)
        except Exception:
            naver_prices = {}

    stats = []
    for sec, stocks in SECTOR_MAP.items():
        # 네이버 실시간 데이터가 있으면 우선 사용, 없으면 df_m 폴백
        naver_hits = [naver_prices[s] for s in stocks if s in naver_prices]
        if naver_hits and len(naver_hits) >= 2:
            # 네이버 실시간 데이터로 계산
            avg_c = sum(h['change'] for h in naver_hits) / len(naver_hits)
            tot_a = sum(h['amount'] for h in naver_hits)  # 이미 억원 단위
            sorted_hits = sorted(
                [(s, naver_prices[s]) for s in stocks if s in naver_prices],
                key=lambda x: x[1]['amount'], reverse=True
            )[:2]
            leaders_str = ', '.join([f"{name}({d['change']:+.1f}%)" for name, d in sorted_hits])
        else:
            # df_m 폴백 (장외 또는 네이버 조회 실패 시)
            if 'ChagesRatio' not in df_m.columns:
                continue
    ...
    <div style="background: linear-gradient(135deg, #1e222d 0%, #161a23 100%); border: 1px solid #2a2e39; border-radius: 10px; padding: 10px 14px; margin-bottom: 14px; font-family: 'malgun gothic', sans-serif;">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
        ...
        <span style="font-size: 11px; color: #00e676; font-weight: bold;">⚡ 09:00~15:30 네이버 실시간 집계</span>
      </div>
    ...
    try:
        ...
        return {
            ...
            'opt': {
                ...
                'guide_action': mp.get('guide_action', '💡 외인 지수 가두리 밴드 (하단 지지 매수 / 상단 저항 익절)')
            }
        }
        ...
        return {
            ...
            'opt': {
                ...
                'guide_action': '💡 외인 지수 가두리 밴드 (하단 지지 매수 / 상단 저항 익절)'
            }
        }
...
try:
    ...
    if c_metrics:
        ...
        if mv.get('active'):
            ...
            v_guide = mv.get('guide_action') or "💡 지수 20일선 이탈! KODEX 200/대형주 분할 줍줍 구간"
            v_border = "#f6465d"
        else:
            ...
            v_guide = mv.get('guide_action') or "💡 극단 투매 시 KODEX 200/대형주 역발상 바닥 매수 타점"
            v_border = "rgba(255,255,255,0.1)"
        v_tooltip = f"[GD 바실리: 국내 코스피 바닥 저격]\n20일선 이격도와 RSI 과매도로 KOSPI 투매 바닥을 저격하는 역발상 매수 신호\n- 현재 상태: {mv.get('stage_name', '정상 관망')}\n- 실시간 가이드: {v_guide.replace('💡 ', '')}"
            
        # 2. BOD 신호 상태 (미국 3대 지수 실시간 분석 기반 동적 타깃 매핑)
        target_s = mb.get('target_short', 'QQQ/SPY 지수 ETF')
        lead_s = mb.get('lead_sector', '지수 전반')
        target_full = mb.get('target_etf', 'QQQ, SPY (미국 대표 지수 ETF)')
        if mb.get('active'):
            ...
            b_guide = mb.get('guide_action') or f"💡 Buy On Dips ({target_s} 1차 분할 줍줍)"
            b_border = "#f39c12"
        else:
            b_badge = "<span style='color:#2ecc71; font-weight:bold;'>🟢 정상 (관망)</span>"
            b_desc = f"미국 지수 고점 낙폭 {mb.get('drawdown', -2.1):.1f}%"
            b_guide = mb.get('guide_action') or f"💡 미국 {target_s} 건전한 조정 시 분할 매수 타점"
            b_border = "rgba(255,255,255,0.1)"
        b_tooltip = f"[GD BOD: 미국 지수 Buy On Dips]\n미국 60일 전고점 대비 낙폭과 섹터별 등락을 실시간 추적하여 건전한 조정 시 분할 매수\n- 현재 주도 조정: {lead_s}\n- 추천 타깃: {target_full}\n- 1단계(-5%): 20% 1차 줍줍\n- 2단계(-8.5%): 30% 2차 매수\n- 3단계(-12%): 패닉 셀링 적극 매수"
        ...
        if vix_val >= 25.0:
            vp_guide = "💡 공포 지수 급등! 변동성 확대 주의 및 현금 비중 확대"
        elif pc_ratio > 1.0:
            vp_guide = "💡 하방 헤지 과열 (풋옵션 우세) · 단기 저점 형성 주시"
        else:
            vp_guide = "💡 헤지펀드 투매 위험 낮음 · 단기 과열 시 분할 익절 대응"
        vp_tooltip = f"[VIX & 풋콜 심리]\n- 실시간 VIX: {vix_val:.2f} ({vix_st}) — 18 미만 안정, 25 이상 공포 경계\n- 풋/콜 비율: {pc_ratio:.2f} — 0.8 이하 상승 기대 우세, 1.0 이상 하방 경계\n- 행동 요령: {vp_guide.replace('💡 ', '')}"
        
        # 4. 옵션 만기일 Max Pain
        d_day_str = f"D-{mopt['d_days']}" if mopt['d_days'] > 0 else "D-Day"
        opt_range = f"{mopt['lower']:,.0f} ~ {mopt['upper']:,.0f}pt"
        opt_guide = mopt.get('guide_action') or "💡 외인 지수 가두리 밴드 (하단 지지 매수 / 상단 저항 익절)"
