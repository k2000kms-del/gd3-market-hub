...
# 09:30 이후면 장초반 변동성이 이미 지나갔으므로 차단 (단, FORCE_SEND=1 이면 수동 발송 허용)
force_send = os.environ.get("FORCE_SEND") == "1"
if now_hm > 930 and not force_send:
    print(f"  ⚠️ 현재 시각({now_hm})이 09:30을 초과하여 장전/장초반 모닝 브리핑 발송을 안전하게 차단합니다.")
    sys.exit(0)
